...
def run_pointnet_only_experiment(train_data: Dict[str, np.ndarray],
    ...
    model = PointNetOnlyClassifier(input_dim=input_dim, num_classes=40)
    
    # Train
    checkpoint_dir = os.path.join(exp_dir, "checkpoints")
    train_losses, test_accs = train_single_feature_model(
        model=model,
...
def run_ulip_only_experiment(train_data: Dict[str, np.ndarray],
    ...
    # Create model
    input_dim = train_features.shape[1]
    model = ULIPOnlyClassifier(input_dim=input_dim, num_classes=40)
...
def main():
    ...
    for exp_name, exp_func in experiments:
        ...
        try:
            final_acc, best_acc = exp_func(train_data, test_data, exp_dir, args, logger)
