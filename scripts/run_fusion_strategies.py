#!/usr/bin/env python
# scripts/run_fusion_strategies.py - Run ablation studies with multiple fusion strategies
import os
import sys
import argparse
import json
...
from fusion_model import PointNetOnlyClassifier, ULIPOnlyClassifier, FusionModel, NormalizedFusionHead, GatedFusionHead
from train_utils import train_single_feature_model, train_fusion_model


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run ablation studies for feature fusion with multiple fusion strategies")
...
def run_fusion_experiment(train_data: Dict[str, np.ndarray],
                          ...
                          fusion_type: str = 'concat') -> Tuple[float, float]:
    """Run experiment using both features (fusion).
    
    Args:
        fusion_type: Type of fusion strategy ('concat', 'normalized', or 'gated')
    """
    logger.info(f"Running {fusion_type} fusion experiment...")
    ...
    results = {
        'experiment': f'fusion_{fusion_type}',
        'final_accuracy': float(final_accuracy),
        'best_accuracy': float(best_accuracy),
        'final_loss': float(train_losses[-1]),
        'train_losses': [float(l) for l in train_losses],
    ...
    logger.info(f"{fusion_type.capitalize()} fusion: final accuracy = {final_accuracy:.4f}, best = {best_accuracy:.4f}")
    return final_accuracy, best_accuracy
...
def main():
    ...
    # Run experiments (pointnet-only, ulip-only, and three fusion strategies)
    experiments = [
        ('pointnet_only', run_pointnet_only_experiment),
        ('ulip_only', run_ulip_only_experiment),
        ('fusion_concat', lambda td, vd, tsd, ed, a, l: run_fusion_experiment(td, vd, tsd, ed, a, l, fusion_type='concat')),
        ('fusion_normalized', lambda td, vd, tsd, ed, a, l: run_fusion_experiment(td, vd, tsd, ed, a, l, fusion_type='normalized')),
        ('fusion_gated', lambda td, vd, tsd, ed, a, l: run_fusion_experiment(td, vd, tsd, ed, a, l, fusion_type='gated'))
    ]
